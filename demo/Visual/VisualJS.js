    ...
    this.Changed = function () {
      ...
      if (!this.IsUpdating() && !(0 in this.FComponentState)) {
        ...
        if ($tmp === 2) {
         ...
         else if ($tmp === 1) $with1.style.setProperty("text-align","right");
...
rtl.module("Unit1",["System","Classes","SysUtils","Forms","Controls","StdCtrls","ExtCtrls","Dialogs","Graphics","Web","WebCtrls","WebCtrlsMore"],function () {
  "use strict";
  var $mod = this;
  var $impl = $mod.$impl;
  rtl.createClass(this,"TForm1",pas.Forms.TForm,function () {
    ...
    this.FormResize = function (Sender) {
      var i1 = 0;
      var i2 = 0;
      i2 = this.FHeight - $impl.ImageContext.HeightDiff;
      if (i2 < 0) i2 = 0;
      i1 = this.FWidth - $impl.ImageContext.WidthDiff;
      if (i1 < 0) i1 = 0;
      this.Image1.SetBounds(this.Image1.FLeft,this.Image1.FTop,i1,i2);
    };
    ...
    this.Button2Click = function (Sender) {
      ...
      $impl.IsTRunning = !$impl.IsTRunning;
      if (!$impl.IsTRunning) {
        this.Timer1.SetEnabled(false);
        $impl.ModifyButton(Sender,true,"&T");
        this.Invalidate();
      } else {
        $impl.ModifyButton(Sender,false,"&End");
        if ($impl.TContext.PalStatus === 0) $impl.TCreatePalette();
        this.Timer1.SetInterval(30);
        this.Timer1.SetEnabled(true);
      };
    };
    this.Button3Click = function (Sender) {
      this.StaticText2.SetVisible(false);
      $impl.IsFireRunning = !$impl.IsFireRunning;
      if (!$impl.IsFireRunning) {
        this.Timer1.SetEnabled(false);
        $impl.ModifyButton(Sender,true,"&Fire");
        this.Invalidate();
      } else {
        this.Label1.SetText("Please wait...");
        this.Label1.SetVisible(true);
        $impl.ModifyButton(Sender,false,"&End");
        this.Timer1.SetInterval(5);
        this.Timer1.SetEnabled(true);
    ...
    this.Button4Click = function (Sender) {
      ...
      this.StaticText2.SetVisible(false);
      $impl.IsImageRunning = !$impl.IsImageRunning;
      if (!$impl.IsImageRunning) {
        this.Label1.SetVisible(false);
        $impl.ModifyButton(Sender,true,"&Image...");
        this.Image1.SetVisible(false);
      } else {
        this.OpenDialog1.Options = rtl.unionSet(this.OpenDialog1.Options,rtl.createSet(8,9));
        this.OpenDialog1.Filter = "Bitmap Files (*.bmp)|*.bmp|PNG Files (*.png)|*.png";
        if (!this.OpenDialog1.Execute()) {
          $impl.IsImageRunning = false;
          return;
        };
        BitmapLoaded = true;
        try {} catch ($e) {
        ...
        if (!BitmapLoaded) {
          pas.Dialogs.ShowMessage$1("*** ERROR ***: Absent or incorrect image file: " + this.OpenDialog1.FileName);
          $impl.IsImageRunning = false;
          return;
        };
        $impl.ModifyButton(Sender,false,"&End");
        this.Label1.SetText("(Resize the form)");
        this.Label1.SetVisible(true);
    ...
    $r.addField("Image1",pas.WebCtrls.$rtti["TImage"]);
    ...
    $r.addMethod("FormPaint",0,[["Sender",pas.System.$rtti["TObject"]]]);
    $r.addMethod("FormResize",0,[["Sender",pas.System.$rtti["TObject"]]]);
  ...
  this.Form1 = null;
  $mod.$implcode = function () {
    rtl.recNewT($impl,"TContext_Struct",function () {
      this.PalStatus = 0;
      this.$eq = function (b) {
        return this.PalStatus === b.PalStatus;
      };
      this.$assign = function (s) {
        this.PalStatus = s.PalStatus;
        return this;
      };
    });
    rtl.recNewT($impl,"ImageContext_Struct",function () {
      this.WidthDiff = 0;
      this.HeightDiff = 0;
      this.$eq = function (b) {
        return (this.WidthDiff === b.WidthDiff) && (this.HeightDiff === b.HeightDiff);
      };
      this.$assign = function (s) {
        this.WidthDiff = s.WidthDiff;
        this.HeightDiff = s.HeightDiff;
        return this;
      };
    });
    $impl.IsTRunning = false;
    $impl.IsFireRunning = false;
    $impl.IsImageRunning = false;
    $impl.TContext = $impl.TContext_Struct.$new();
    $impl.ImageContext = $impl.ImageContext_Struct.$new();
    $impl.ModifyButton = function (CurButton, State, NewCaption) {
      if ($mod.Form1.Button1 === CurButton) {
        $mod.Form1.Button1.SetText(NewCaption)}
       else $mod.Form1.Button1.SetEnabled(State);
      if ($mod.Form1.Button2 === CurButton) {
        $mod.Form1.Button2.SetText(NewCaption)}
       else $mod.Form1.Button2.SetEnabled(State);
      if ($mod.Form1.Button3 === CurButton) {
        $mod.Form1.Button3.SetText(NewCaption)}
       else $mod.Form1.Button3.SetEnabled(State);
      if ($mod.Form1.Button4 === CurButton) {
        $mod.Form1.Button4.SetText(NewCaption)}
       else $mod.Form1.Button4.SetEnabled(State);
    };
    $impl.TCreatePalette = function () {
    };
  };
},[]);
rtl.module("U1",["System","SysUtils","Classes","Dialogs","Controls","StdCtrls","Forms","Graphics","WebCtrls","WebCtrlsMore"],function () {
  "use strict";
  var $mod = this;
  this.Loaded = function () {
    ...
    pas.Unit1.Form1.SetLeft(131);
    pas.Unit1.Form1.SetHeight(514);
    pas.Unit1.Form1.SetTop(20);
    pas.Unit1.Form1.SetWidth(700);
    pas.Unit1.Form1.SetText("Visual - GDI and OpenGL tests");
    pas.Unit1.Form1.FOnCreate = rtl.createCallback($with,"FormCreate");
    pas.Unit1.Form1.FOnDestroy = rtl.createCallback($with,"FormDestroy");
    pas.Unit1.Form1.FOnPaint = rtl.createCallback($with,"FormPaint");
    pas.Unit1.Form1.FOnResize$1 = rtl.createCallback($with,"FormResize");
    ...
    $with.Button1.SetLeft(464);
    $with.Button1.SetHeight(46);
    $with.Button1.SetTop(8);
    $with.Button1.SetWidth(82);
    ...
    $with.Button2.SetLeft(464);
    $with.Button2.SetHeight(46);
    $with.Button2.SetTop(64);
    $with.Button2.SetWidth(82);
    $with.Button2.SetText("&T");
    ...
    $with.Button3.SetLeft(464);
    $with.Button3.SetHeight(46);
    $with.Button3.SetTop(120);
    $with.Button3.SetWidth(82);
    $with.Button3.SetText("&Fire");
    $with.Button3.FOnClick = rtl.createCallback($with,"Button3Click");
    $with.Button3.EndUpdate();
    $with.Image1 = pas.WebCtrls.TImage.$create("Create$1",[pas.Unit1.Form1]);
    $with.Image1.BeginUpdate();
    $with.Image1.SetParent(pas.Unit1.Form1);
    $with.Image1.SetLeft(24);
    $with.Image1.SetHeight(337);
    $with.Image1.SetTop(64);
    $with.Image1.SetWidth(424);
    $with.Image1.SetStretch(true);
    $with.Image1.SetVisible(false);
    $with.Image1.EndUpdate();
    ...
    $with.StaticText1.SetLeft(464);
    $with.StaticText1.SetHeight(30);
    $with.StaticText1.SetTop(384);
    $with.StaticText1.SetWidth(150);
    ...
    $with.Button4.SetLeft(464);
    $with.Button4.SetHeight(46);
    $with.Button4.SetTop(176);
    $with.Button4.SetWidth(82);
    $with.Button4.SetText("&Image...");
    ...
    $with.Label1.SetLeft(49);
    $with.Label1.SetHeight(31);
    $with.Label1.SetTop(17);
    $with.Label1.SetWidth(367);
    $with.Label1.SetAlignment(2);
    $with.Label1.SetAutoSize(false);
    $with.Label1.SetColor(8421504);
    $with.Label1.FFont.SetColor(0);
    $with.Label1.FFont.SetHeight(-21);
    $with.Label1.FFont.SetName("Tahoma");
    $with.Label1.SetParentColor(false);
    $with.Label1.SetParentFont(false);
    $with.Label1.SetVisible(false);
    ...
    $with.StaticText2.SetLeft(49);
    $with.StaticText2.SetHeight(180);
    $with.StaticText2.SetTop(120);
    $with.StaticText2.SetWidth(367);
    ...
    $with.OpenDialog1.SetLeft(16);
    $with.OpenDialog1.SetTop(56);
    $with.OpenDialog1.EndUpdate();
    pas.Unit1.Form1.EndUpdate();
  };
},["Unit1"]);
rtl.module("program",["System","Unit1","U1","Forms"],function () {
  "use strict";
  var $mod = this;
  $mod.$main = function () {
...
//# sourceMappingURL=VisualJS.js.map
